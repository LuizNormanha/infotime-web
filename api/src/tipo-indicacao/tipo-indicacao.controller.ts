...
@Controller('tipos-indicacao')
export class TipoIndicacaoController {
  constructor(private readonly service: TipoIndicacaoService) {}
  ...
  @Post()
  criar(
    @Body() dto: CriarTipoIndicacaoDto,
    @TenantAtual() idTenacidade: bigint,
    @UsuarioAtual() idUsuario: bigint,
    @Req() req: Request,
  ) {
  ...
  atualizar(
    @Param('id') id: string,
    @Body() dto: AtualizarTipoIndicacaoDto,
    @TenantAtual() idTenacidade: bigint,
    @UsuarioAtual() idUsuario: bigint,
    @Req() req: Request,
  ) {
