...
import { RespostaListagemTipoEstadoCivilDto } from './dto/resposta-listagem-tipo-estado-civil.dto';
import { RespostaTipoEstadoCivilDto } from './dto/resposta-tipo-estado-civil.dto';
...
export class TipoEstadoCivilService {
  ...
  private whereCampoPesquisa(
    ...
    qTexto: string,
  ): Prisma.infolab_tipo_estado_civilWhereInput {
    ...
    if (campoPesquisa === 'id') {
      try {
        return { id_tipo_estado_civil: BigInt(qTexto.trim()) };
      } catch {
        return {};
      }
    }
  ...
  private whereFiltroRefinado(
    ...
    const partes: Prisma.infolab_tipo_estado_civilWhereInput[] = [];
  ...
  async listar(
  ...
  ): Promise<{ dados: RespostaListagemTipoEstadoCivilDto[]; total: number }> {
    void tenantContexto;
    const baseWhere: Prisma.infolab_tipo_estado_civilWhereInput = {};
    ...
    return executarListagemCrudCatalogo({
      ...
      delegate: this.prisma.infolab_tipo_estado_civil,
      baseWhere,
      camposPesquisaWhitelist: TipoEstadoCivilService.CAMPOS_PESQUISA,
      montarWhereCampoPesquisa: (campo, q) => this.whereCampoPesquisa(campo, q),
      montarWhereFiltroRefinado: (j) => this.whereFiltroRefinado(j),
      orderBy: { id_tipo_estado_civil: 'desc' },
      skipTakeSelect: { skip: 0, take: 10, select },
      mapRow: (row: unknown) => {
        const r = row as {
          id_tipo_estado_civil: bigint;
          descricao: string | null;
        };
        return {
          id: r.id_tipo_estado_civil.toString(),
          descricao: r.descricao ?? null,
        };
      },
      findManyLegado: ({ where, orderBy, select: sel, take }) =>
        this.prisma.infolab_tipo_estado_civil.findMany({
          where: where as Prisma.infolab_tipo_estado_civilWhereInput,
          orderBy,
  ...
  async buscarPorId(
    id: string,
    tenantContexto: TenantContexto,
  ): Promise<{ dados: RespostaTipoEstadoCivilDto }> {
    void tenantContexto;
    const registro = await this.prisma.infolab_tipo_estado_civil.findUnique({
      where: { id_tipo_estado_civil: BigInt(id) },
    });

    if (!registro) {
      throw new NotFoundException(`Tipo de estado civil ${id} não encontrado.`);
    }
  ...
  async criar(
    dto: CriarTipoEstadoCivilDto,
    tenantContexto: TenantContexto,
    ip: string,
  ): Promise<{ id: string }> {
    const criado = await this.prisma.infolab_tipo_estado_civil.create({
      data: {
      ...
      select: { id_tipo_estado_civil: true },
    });

    return { id: criado.id_tipo_estado_civil.toString() };
  }

  async atualizar(
    id: string,
    dto: AtualizarTipoEstadoCivilDto,
    tenantContexto: TenantContexto,
    ip: string,
  ): Promise<{ id: string }> {
    const existente = await this.prisma.infolab_tipo_estado_civil.findUnique({
      where: { id_tipo_estado_civil: BigInt(id) },
      select: { id_tipo_estado_civil: true },
    });
    ...
    await this.prisma.infolab_tipo_estado_civil.update({
      where: { id_tipo_estado_civil: BigInt(id) },
      data: {
        id_usuario_auditoria: tenantContexto.idUsuario,
        endereco_ip_auditoria: truncarIp(ip, 20),
  ...
  async excluir(
    ...
    try {
      await this.prisma.infolab_tipo_estado_civil.delete({
        where: { id_tipo_estado_civil: BigInt(id) },
      });
    } catch (e: unknown) {
      if (
        ...
        throw new ConflictException(
          'Não é possível excluir: existem registros vinculados a este tipo de estado civil.',
        );
      }
      throw e;
    }
  ...
  }): RespostaTipoEstadoCivilDto {
    return {
      id: registro.id_tipo_estado_civil.toString(),
