...
@Controller('tipos-interface')
export class TipoInterfaceController {
  constructor(private readonly service: TipoInterfaceService) {}
  ...
  @Post()
  criar(
    @Body() dto: CriarTipoInterfaceDto,
    @TenantAtual() idTenacidade: bigint,
    @UsuarioAtual() idUsuario: bigint,
    @Req() req: Request,
  ) {
  ...
  atualizar(
    @Param('id') id: string,
    @Body() dto: AtualizarTipoInterfaceDto,
    @TenantAtual() idTenacidade: bigint,
    @UsuarioAtual() idUsuario: bigint,
    @Req() req: Request,
  ) {
