...
const IP_MAX = 50;
/** Situação “Pago” no legado Scriptcase (RN-06). */
const ID_SITUACAO_DOCUMENTO_PAGO = 4n;
/** Espécie cheque no legado (RN-07, RN-09). */
const ID_TIPO_ESPECIE_CHEQUE = 1n;
...
} as const satisfies Prisma.infotime_lancamento_despesaSelect;

export type ContasPagarListaItemDto = Record<string, unknown>;
...
export class ContasPagarService {
  ...
  private whereCampoPesquisa(
    ...
    switch (campoPesquisa) {
      ...
      case 'idLancamentoDespesa':
        try {
          return { id_lancamento_despesa: BigInt(q) };
        } catch {
          return {};
        }
      default:
  ...
  private async whereCampoPesquisaAsync(
    ...
    if (campoPesquisa === 'nomeAgente') {
      ...
      const partes: Prisma.infotime_lancamento_despesaWhereInput[] = [
  ...
  private whereDespesaListagemDrilldown(
    ...
    const naoPago: Prisma.infotime_lancamento_despesaWhereInput = {
      OR: [
    ...
    return {
      AND: [{ data_previsao: { lt: this.inicioDiaCivilAtual() } }, naoPago],
    };
  }
  ...
  private async enriquecerLinhasLista(
    ...
    linhas: Array<{
      ...
      valor_previsao_liquido: Prisma.Decimal | null;
    ...
    return linhas.map((r) => {
      ...
      return {
        ...
        valorPrevisaoLiquido: this.decParaJson(r.valor_previsao_liquido),
  ...
  async listar(
    ...
    todos?: boolean,
    query?: QueryListagemCrudPadrao,
  ): Promise<{ dados: ContasPagarListaItemDto[]; total: number }> {
    const baseWhere: Prisma.infotime_lancamento_despesaWhereInput = {
      id_tenacidade: tenantContexto.idTenacidade,
    };
    const takeLegado = 500;

    if (!modoListagemCrudNovo(query)) {
      const linhas = await this.prisma.infotime_lancamento_despesa.findMany({
        where: baseWhere,
  ...
  /** RN-01 — alteração: registro já marcado com fechamento. */
  private assertRegistroNaoFechadoParaAlteracao(
    idTenacidade: bigint,
    fechamentoFinanceiroAtual: string | null | undefined,
  ): void {
    if (idTenacidade !== 1n) return;
    if ((fechamentoFinanceiroAtual ?? '').trim().toUpperCase() === 'S') {
      throw new BadRequestException(
        'Despesa não pode ser alterada. Já houve fechamento financeiro para esse período.',
      );
    }
  }

  /** RN-01 — inclusão: período de `data_previsao` coberto por fechamento financeiro. */
  private async assertPeriodoLivreParaInclusao(
    ...
    if (row.length > 0) {
      throw new BadRequestException(
        'Despesa não pode ser incluída. Já houve fechamento financeiro para esse período.',
      );
    }
  }
  ...
  private async assertPlanoContaTenant(
    ...
    const rows = await this.prisma.$queryRaw<{ ok: bigint }[]>`
      ...
      WHERE id_plano_conta = ${idPlanoConta} AND id_tenacidade = ${idTenacidade}
      LIMIT 1
    `;
    if (rows.length === 0) {
      throw new BadRequestException('Plano de Contas: Campo obrigatório');
    }
  }
  ...
  private aplicarValidacoesPagamento(
    ...
    if (idSituacao === ID_SITUACAO_DOCUMENTO_PAGO) {
    ...
    if (idTipoEspecie === ID_TIPO_ESPECIE_CHEQUE && idCheque == null) {
      throw new BadRequestException('Campo obrigatório: Cheque');
    }
  }
  ...
  private patchUpdate(
    ...
    if (dto.valorPrevisaoLiquido !== undefined) {
      patch.valor_previsao_liquido = this.parseDecimalOpcional(
        dto.valorPrevisaoLiquido,
        'valorPrevisaoLiquido',
      );
    }
    ...
    if (dto.idTipoEspecie !== undefined) {
      const idTe = this.parseBigIntOpcional(dto.idTipoEspecie, 'idTipoEspecie');
      patch.id_tipo_especie = idTe;
      if (idTe !== ID_TIPO_ESPECIE_CHEQUE) {
        patch.id_cheque = null;
      }
    }
    if (dto.idCheque !== undefined) {
      patch.id_cheque = this.parseBigIntOpcional(dto.idCheque, 'idCheque');
    }
    if (dto.numeroDocumento !== undefined) {
      patch.numero_documento = dto.numeroDocumento ?? null;
    }
    if (dto.dataAgendamento !== undefined) {
      patch.data_agendamento = this.parseDataHoraOpcional(
        dto.dataAgendamento,
        'dataAgendamento',
      );
    }
  ...
  async obterPorId(
    ...
    const registro = await this.prisma.infotime_lancamento_despesa.findFirst({
      where: {
        id_lancamento_despesa: idLanc,
        id_tenacidade: tenantContexto.idTenacidade,
      },
    });
  ...
  async criar(
    ...
    const criado = await this.prisma.infotime_lancamento_despesa.create({
      data,
      select: { id_lancamento_despesa: true },
    });
    return { id: criado.id_lancamento_despesa.toString() };
  }
  ...
  async atualizar(
    ...
    const atual = await this.prisma.infotime_lancamento_despesa.findFirst({
      where: {
        id_lancamento_despesa: idLanc,
        id_tenacidade: tenantContexto.idTenacidade,
      },
    ...
    if (idSit === ID_SITUACAO_DOCUMENTO_PAGO) {
      patch.id_usuario_realizacao = tenantContexto.idUsuario;
      patch.id_usuario_baixa = tenantContexto.idUsuario;
    }

    await this.prisma.infotime_lancamento_despesa.update({
      where: { id_lancamento_despesa: idLanc },
      data: {
        ...patch,
        id_usuario_auditoria: tenantContexto.idUsuario,
        endereco_ip_auditoria: this.fatiarIp(ip),
  ...
  async excluir(id: string, tenantContexto: TenantContexto): Promise<void> {
    ...
    const existe = await this.prisma.infotime_lancamento_despesa.findFirst({
      where: {
        id_lancamento_despesa: idLanc,
        id_tenacidade: tenantContexto.idTenacidade,
      },
    ...
    try {
      await this.prisma.infotime_lancamento_despesa.delete({
        where: { id_lancamento_despesa: idLanc },
      });
    } catch (e: unknown) {
      const code =
