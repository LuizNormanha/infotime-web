-- Resolução de tenant por domínio no login (SECURITY DEFINER ignora RLS).
-- InfoTIME ERP: `tenacidade` + `usuario.email`; Liga: `infolab_tenacidade` + `infolab_tenacidade_configuracao`.
