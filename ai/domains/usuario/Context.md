...
Este domínio fornece a base de regras para APIs, UIs e APIs e UIs que manipulam usuários (cadastro, edição, troca de senha).
...
Representa uma unidade física/organizacional (ex.: unidade de atendimento, laboratório filiado) à qual o usuário está associado.
...
- Telas de cadastro de usuário na web devem seguir as regras deste domínio (`ai/domains/usuario`) e o padrão global de layout/foco em **`ai/domains/padroes-ui`** (§8 — ordem do layout, `data-campo-chave` em lookups customizados, troca de seção na sidebar).
...
o domínio/Usuário não dita esses detalhes finos (como “mínimo 8 caracteres, uma maiúscula”), mas garante que:
