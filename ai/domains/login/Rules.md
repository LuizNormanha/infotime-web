...
1.2. A Tenacidade deve ser identificada consultando a configuração canônica em `tenacidade_configuracao` pelo campo `dominio_tenacidade`.
...
- Buscar o usuário em `usuario` pela combinação (`id_tenacidade` + `login` informado).
...
- A senha informada deve ser validada comparando o valor informado com o hash armazenado (`senha_hash`), utilizando algoritmo seguro.
...
3.5.5. A implementação de CAPTCHA deve seguir boas práticas de UX e acessibilidade, evitando tornar o login inviável para usuários legítimos, mas protegendo contra ataques de força bruta.

## 4. Verificação de sessão ativa para usuário comum

4.1. Antes de criar nova sessão para o usuário, o sistema deve consultar `sessao_usuario` para verificar se existe sessão ativa para aquele usuário (naquele tenant).
...
- Localize em `sessao_usuario` todas as sessões marcadas como ativas, mas cuja validade (tempo de expiração) já tenha sido ultrapassada.
- Marque essas sessões como inativas/encerradas.

5.2. Após a limpeza:

- O sistema deve contar quantas sessões ativas existem para a Tenacidade em `sessao_usuario`.
...
- O sistema deve criar uma nova entrada em `sessao_usuario` para o usuário logado.
...
- O sistema deve tratar como login de Suporte/Implantação, independente de existir ou não registro correspondente em `usuario`.
...
- Deve ser criado registro em `sessao_suporte`, registrando:
...
6.5. Usuários de Suporte/Implantação **podem ter múltiplas sessões simultâneas**; não se aplicam a eles as limitações de `sessao_usuario` e limite de licenças.
...
7.4. As informações fornecidas devem ser associadas ao registro de sessão em `sessao_suporte`.
...
## 10. Reautenticação na tela de Registro Coletor

10.1. A tela de **registro_coletor** exige reautenticação, independentemente do estado da sessão atual.

10.2. Se, na reautenticação, o usuário informar **o mesmo login** da sessão atual:

- O sistema deve:
  - Validar a senha.
  - Manter a sessão atual.
  - Redirecionar para a tela de registro coletor.

10.3. Se o usuário informar **login diferente**:

- O sistema deve:
  - Encerrar a sessão atual (logout completo).
  - Criar nova sessão para o novo usuário (respeitando todas as regras de login comum ou especial, conforme o caso).
  - Redirecionar para a tela de registro coletor.

10.4. As regras detalhadas de uso da tela de registro coletor serão definidas em `ai/domains/registro-coletor` quando o produto adotar esse domínio.
...
- Trocas de usuário em reautenticação (especialmente na tela de registro coletor).
