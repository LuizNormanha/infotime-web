...
- Tabela: `infolab_tenacidade_configuracao` (Prisma: `infolab_tenacidade_configuracao`), ligada a `infolab_tenacidade` por `id_tenacidade`.
...
- Representado por `infolab_usuario`.
...
- Representada por `infolab_sessao_usuario`.
...
- Representada por `infolab_sessao_suporte`.
...
- Tela específica (domínio `registro_coletor`, detalhado em documento próprio em `ai/domains`).
...
2. Sistema resolve a Tenacidade:
   - Localiza `infolab_tenacidade_configuracao` pelo `dominio_tenacidade`.
...
4. Após Tenacidade válida:
   - Busca usuário em `infolab_usuario` pela combinação (`id_tenacidade`/Tenacidade + `login`).
   - Valida senha do usuário.

5. Se senha válida:
   - Verifica sessões ativas em `infolab_sessao_usuario` para o usuário.
...
6. Controle de licença simultânea:
   - Antes de criar a nova sessão:
     - Executa rotina que remove de `infolab_sessao_usuario` todas as sessões já expiradas, mas ainda marcadas como ativas.
   - Depois da limpeza:
...
- Ignora Tenacidade específica no `infolab_usuario`.
...
- Se a senha do dia estiver correta:
  - Login é permitido.
  - É registrado um novo registro em `infolab_sessao_suporte`.
...
### 5. Reautenticação em fluxos sensíveis

Alguns módulos podem exigir confirmação de credenciais antes de prosseguir. O comportamento geral (modal, troca de usuário, retomada da ação) está em `login/Rules.md` §9–§10; detalhes por módulo entram nos respectivos domínios quando forem criados em `ai/domains/`.