...
1. **Criação de Tenacidade**
   ...
   - Qualquer outro usuário, inclusive de outros domínios que não sejam `liga.br`, **não pode** criar/akterar/excluir/visualizar Tenacidades.

2. **Alteração de Tenacidade**
   - Apenas `suporte@liga.br` e `implantacao@liga.br` podem alterar Tenacidades existentes.
   - Usuários fora do domínio `liga.br` ou com e-mails diferentes destes dois **não podem** alterar Tenacidades.
...
2. **Renovação**
   - A data de expiração é renovada somente mediante confirmação de pagamento em dia (processo comercial definido fora deste repositório, até existir domínio dedicado).
...
1. **Geração Automática**
   - `chave_jwt` deve ser gerada automaticamente no ato da criação da Tenacidade (persistida em **`infolab_tenacidade_configuracao`**, linha canônica do laboratório).
...
4. **Persistência**
   - Domínio, expiração da licença e `chave_jwt` residem na **configuração da tenacidade**, não na tabela `infolab_tenacidade` isolada.
...
## Listagem de implantação (`implantacao-tenacidade`)

- A listagem no shell segue `ai/domains/padroes-ui/Rules.md` §11: **sem** subtítulo de topo salvo exceção (§3.5); o vínculo com configuração do tenant aparece nas colunas e no domínio **tenacidade**, não como parágrafo redundante sob o título.
