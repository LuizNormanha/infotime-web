...
Este documento descreve **layout por seções**, **disciplina de texto e UX** (rótulos, seções, agrupamentos, subtítulo do topo), **tipos de campo**, **validação** (incluindo máscaras/dígitos), **auditoria**, **ações (Cancelar / Salvar / Excluir)** e **integração BFF**, alinhado ao código atual do frontend. O exemplo principal é o cadastro de **unidade de atendimento**; outras features reutilizam os mesmos building blocks mudando **`LayoutFormularioCadastro`**, **`substituirRenderCampo`**, **`secoesCustom`**, **endpoint** e **i18n**. Para **lookup + cartão de resumo em `fieldset`** (ex.: cliente / convênio / contato), ver **[`cartao-selecao.md`](cartao-selecao.md)** (também no `Rules.manifest.json`). Para **endereço brasileiro com CEP automático (ViaCEP), coordenadas e mapa OSM**, ver **[`endereco-cep-geocode-mapa.md`](endereco-cep-geocode-mapa.md)**. Para **alinhamento de colunas entre linhas e espaçamento exclusivamente com tokens do módulo** (referência: cliente Infotime Liga), ver **[`alinhamento-espacamento-formulario.md`](alinhamento-espacamento-formulario.md)**. Para **botões de contorno no topo** (altura ~40px, cápsula, tokens CSS), ver **[`../../ui/botoes-listagem-formulario.md`](../../ui/botoes-listagem-formulario.md)**. Para **checklist único** (listagem + cadastro, foco, remissões), ver **[`normas-transversais-ui-cadastro.md`](normas-transversais-ui-cadastro.md)**. Para **modais de confirmação** (voltar com alterações pendentes, excluir sem `window.confirm`, `Dialog` + `liga-mensagem-pop-up`), ver **[`dialogos-confirmacao-formulario.md`](dialogos-confirmacao-formulario.md)**.
...
- **Exclusão:** só com **`idEdicao`**; confirmação via **`LigaMensagemPopUp`** (`home.formulario.tituloConfirmarExclusaoCadastro`, `confirmarExclusaoCadastro`, `botaoConfirmarExclusaoCadastro`). **Formulários Liga** que não usem esse pop-up (ex.: mais de duas ações ou alinhamento ao padrão visual dos modais `liga-mensagem-pop-up`) devem confirmar exclusão com **`Dialog`** conforme **[`dialogos-confirmacao-formulario.md`](dialogos-confirmacao-formulario.md)** — **nunca** `window.confirm`.
