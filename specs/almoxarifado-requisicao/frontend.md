# Frontend — almoxarifado-requisicao

## Páginas

- `apps/web/src/pages/almoxarifado-requisicao/` — listagem e formulário *(nomes PascalCase na implementação)*.

## Componentes

- Reutilizar `ListShell`, `DataTablePage`, `FormShell`, fields em `shared/components/`.

## Dados

- TanStack Query + contratos alinhados a [`api.md`](api.md).

## Visual

- Referência: screenshots do corpus e [`telas.md`](telas.md).
- Padrões visuais globais: [`docs/PADRAO_LISTAS.md`](../../docs/PADRAO_LISTAS.md), [`docs/PADRAO_FORMULARIOS.md`](../../docs/PADRAO_FORMULARIOS.md).
