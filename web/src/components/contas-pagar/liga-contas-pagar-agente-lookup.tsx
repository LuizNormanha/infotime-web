...
async function buscarColaboradores(
  ...
  params.set("q", q);
  const url = `/api/contas-pagar/colaboradores?${params.toString()}`;
...
export type LigaContasPagarAgenteLookupProps = {
  tipoAgente: "1" | "2" | "3";
...
export function LigaContasPagarAgenteLookup({
  tipoAgente,
  rotuloExibicao,
  onInputRotulo,
  onSelecionar,
  ...
  placeholder,
  ariaLabel,
}: LigaContasPagarAgenteLookupProps) {
    ...
    <div className="liga-cliente-infotime-campo-primeira-linha">
      ...
      <div className="liga-cliente-infotime-primeira-linha-controles">
        <AutoComplete
          key={tipoAgente}
          inputId={inputId}
          className="w-full liga-contas-pagar-agente-lookup"
          ...
          itemTemplate={(item: SugestaoAgente) => (
            <span className="liga-contas-pagar-agente-lookup-item">{item.label}</span>
          )}
        />
      </div>
    </div>
