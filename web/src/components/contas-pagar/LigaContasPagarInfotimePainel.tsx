...
import { CONTAS_PAGAR_INFOTIME_COLUNAS_LISTAGEM } from "./contas-pagar-infotime-listagem-colunas";
import "./liga-contas-pagar-infotime.css";
import { LigaContasPagarInfotimeFormulario } from "./LigaContasPagarInfotimeFormulario";
...
const ID_TITULO_LISTAGEM = "liga-contas-pagar-infotime-titulo-listagem";

function ListaContasPagarInfotimeInterna({
  aoAbrirCadastro,
  aoNovo,
  ...
  const t = useTranslations("home.contasPagarInfotime");
  const tLista = useTranslations("home.contasPagarInfotime.lista");
  ...
  const listagemHook = useListagemCrudServidor({
    resourcePath: "/api/contas-pagar",
    campoPesquisaInicial: "nomeAgente",
    linhasPorPaginaInicial: 10,
    queryExtraFixo: queryListagemExtra,
  });
  const colunas = useMemo(() => CONTAS_PAGAR_INFOTIME_COLUNAS_LISTAGEM, []);
  ...
  return (
    <LigaListagemBase
      nomeTabela="infotime_lancamento_despesa"
      codigoTela="contas-pagar"
      listarTodos={false}
      registros={listagemHook.registros}
      colunas={colunas}
      chavePrimaria="idLancamentoDespesa"
...
export function LigaContasPagarInfotimePainel({
  idTenacidade,
  queryListagemExtra,
  ...
  useEffect(() => {
    ...
    try {
      const raw = sessionStorage.getItem(chaveStorageIntentPainel("contas-pagar"));
      if (!raw) return;
  ...
  useEffect(() => {
    ...
    const aoSincronizar = (ev: Event) => {
      ...
      if (det?.alvo !== "contas-pagar") return;
  ...
  const abrirEdicao = useCallback(
    (linha: Record<string, unknown>) => {
      const id = String(linha.idLancamentoDespesa ?? "").trim();
      if (!id) return;
      if (navegacaoBasePath) {
  ...
  if (fluxo === "formulario") {
    return (
      <div className="liga-home-modulo-vazio liga-cliente-infotime-painel liga-contas-pagar-infotime-painel">
        <LigaContasPagarInfotimeFormulario
          idLancamentoDespesa={idEdicao}
  ...
  return (
    <div className="liga-home-modulo-vazio liga-cliente-infotime-painel liga-contas-pagar-infotime-painel">
        ...
        <header className="liga-home-modulo-cabecalho">
          <div className="liga-home-modulo-titulo-linha liga-cliente-infotime-modulo-titulo-linha">
            <div className="liga-cliente-infotime-modulo-titulo-esquerda">
              ...
              >
                <i className="pi pi-minus-circle liga-home-modulo-titulo-icone" aria-hidden />
                {t("lista.titulo")}
              </h1>
            </div>
            <div
              ref={refHostCabecalhoAcoes}
              className="liga-contas-pagar-infotime-cabecalho-acoes-host"
      ...
      <ListaContasPagarInfotimeInterna
