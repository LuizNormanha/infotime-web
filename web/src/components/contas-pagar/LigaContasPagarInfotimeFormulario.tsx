...
import { resolverRotuloAgenteExibicao } from "./contas-pagar-agente-resolver";
import {
  type CamposContasPagar,
  criarSecoesFormularioContasPagarInfotime,
} from "./liga-contas-pagar-infotime-formulario-secoes";
import "./liga-contas-pagar-infotime.css";

const ID_SITUACAO_PAGO = "4";
const ID_TIPO_ESPECIE_CHEQUE = "1";
...
function camposVazio(): CamposContasPagar {
  return {
    idTipoAgente: "2",
...
function montarPayload(c: CamposContasPagar): Record<string, unknown> {
  ...
  if (c.valorPrevisaoLiquido.trim()) o.valorPrevisaoLiquido = c.valorPrevisaoLiquido.trim();
  ...
  if (te === ID_TIPO_ESPECIE_CHEQUE && c.idCheque.trim()) {
    o.idCheque = c.idCheque.trim();
  }
  if (te !== ID_TIPO_ESPECIE_CHEQUE && c.numeroDocumento.trim()) {
    o.numeroDocumento = c.numeroDocumento.trim();
  }
  if (c.dataAgendamento.trim()) o.dataAgendamento = c.dataAgendamento.trim();
...
function camposDeApi(d: Record<string, unknown>): CamposContasPagar {
  return {
    ...
    valorPrevisaoLiquido: str(d.valor_previsao_liquido),
    ...
    idCheque: str(d.id_cheque),
    numeroDocumento: str(d.numero_documento),
    dataAgendamento: isoDateOnly(d.data_agendamento),
...
export function LigaContasPagarInfotimeFormulario({
  idLancamentoDespesa,
  aoVoltar,
  aoSalvo,
}: Props) {
  const t = useTranslations("home.contasPagarInfotime.formulario");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/contas-pagar/lookups", {
          signal: ac.signal,
          cache: "no-store",
        });
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(
          `/api/contas-pagar/${encodeURIComponent(idLancamentoDespesa)}`,
        ...
        setAuditoriaLinhas([
          { label: t("audDataInclusao"), valor: str(d.data_inclusao) },
          { label: t("audIdUsuarioInclusao"), valor: str(d.id_usuario_inclusao) },
          { label: t("audIdUsuarioBaixa"), valor: str(d.id_usuario_baixa) },
          { label: t("audIdContaCaixaRegistro"), valor: str(d.id_conta_caixa_registro) },
          {
            label: t("audIdContaCaixaTransferencia"),
            valor: str(d.id_conta_caixa_transferencia),
          },
          { label: t("audIdLancamentoDespesaPai"), valor: str(d.id_lancamento_despesa_pai) },
          {
            label: t("audIdLancamentoDespesaRecorrente"),
            valor: str(d.id_lancamento_despesa_recorrente),
          },
          { label: t("audFechamentoFinanceiro"), valor: str(d.fechamento_financeiro) },
        ]);
        setArquivosLinhas([
          { label: t("arquivoBoleto"), valor: str(d.nome_referencia_boleto) },
          { label: t("arquivoNf"), valor: str(d.nome_referencia_nota_fiscal) },
          { label: t("arquivoComprovante"), valor: str(d.nome_referencia_comprovante) },
          { label: t("arquivoAnexo"), valor: str(d.nome_referencia_anexo) },
        ]);
      } catch {
    ...
    return () => ac.abort();
  }, [idLancamentoDespesa, aoVoltar, feedback, t]);
  ...
  const situacaoEhPago = campos.idSituacaoDocumento === ID_SITUACAO_PAGO;
  const especieEhCheque = campos.idTipoEspecie === ID_TIPO_ESPECIE_CHEQUE;
  ...
  const validar = useCallback((): Record<string, string> | null => {
    ...
    if (especieEhCheque && !campos.idCheque.trim()) e.idCheque = t("obrigatorio");
    return Object.keys(e).length ? e : null;
  }, [campos, especieEhCheque, t]);
  ...
  const salvar = useCallback(async () => {
    ...
    try {
      ...
      const url = idLancamentoDespesa
        ? `/api/contas-pagar/${encodeURIComponent(idLancamentoDespesa)}`
        : "/api/contas-pagar";
      const method = idLancamentoDespesa ? "PUT" : "POST";
  ...
  }, [aoVoltar, feedback, idLancamentoDespesa, t]);
  ...
  const subtituloFormulario = idLancamentoDespesa
    ? t("subtituloCabecalhoEditar")
    : t("subtituloCabecalhoNovo");

  return (
    <section
      className={`liga-cliente-infotime-form liga-contas-pagar-infotime-form${idLancamentoDespesa ? "" : " liga-cliente-infotime-form--inclusao"}`}
