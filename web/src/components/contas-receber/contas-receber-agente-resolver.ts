import type { CamposContasReceber } from "./liga-contas-receber-infotime-formulario-secoes";
...
export async function resolverRotuloAgenteExibicao(
  ...
  if (ta === "3" && c.idColaborador.trim()) {
    const res = await fetch(
      `/api/contas-receber/colaboradores/${encodeURIComponent(c.idColaborador.trim())}`,
      { signal, cache: "no-store" },
    );
    if (!res.ok) return "";
    const j = (await res.json()) as { dados?: Record<string, unknown> };
